		...
		inline std::error_category &rsa_error_category()
		{
			...
			public:
				...
				std::string message(int ev) const override
				{
					...
					case rsa_error::set_rsa_failed:
						return "set modulus and exponent to RSA failed";
					case rsa_error::create_context_failed:
			...
			public:
				...
				std::string message(int ev) const override
				{
					...
					case ecdsa_error::invalid_key:
						return "invalid key";
					case ecdsa_error::create_context_failed:
						return "failed to create context";
					case ecdsa_error::cert_load_failed:
						return "error loading cert into memory";
					case ecdsa_error::get_key_failed:
						return "error getting key from certificate";
					case ecdsa_error::write_key_failed:
						return "error writing key data in PEM format";
					case ecdsa_error::write_cert_failed:
						return "error writing cert data in PEM format";
					case ecdsa_error::convert_to_pem_failed:
						return "failed to convert key to pem";
					case ecdsa_error::unknown_curve:
						return "unknown curve";
					case ecdsa_error::set_ecdsa_failed:
...
namespace jwt
{
	...
	namespace helper
	{
		...
		private:
			...
			{
				if (m_key != nullptr && EVP_PKEY_up_ref(m_key) != 1)
...
// C++11 requires the body of a constexpr constructor to be empty
		...
		private:
			...
			{
				if (key != nullptr && EVP_PKEY_up_ref(key) != 1)
					throw std::runtime_error("EVP_PKEY_up_ref failed");
			}
													...
													std::error_code &ec)
		{
			...
			if (!pkey)
				ec = error_category::load_key_bio_read;
			return pkey;
		}
	...
	public:
		...
		JWT_CLAIM_EXPLICIT jwks(const typename json_traits::string_type &str)
		{
			...
			std::transform(jwk_list.begin(), jwk_list.end(), std::back_inserter(jwk_claims),
						   [](const typename json_traits::value_type &val)
						   { return jwks_t{val}; });
		}
